  ...
  while ((newLineIndex = this._buffer.indexOf('\n')) !== -1) {
    ...
    if ((found = line.match(/^adapterState (.*)$/))) {
      ...
      if (adapterState === 'unauthorized') {
        debug('bleno warning: adapter state unauthorized, please run as root or with sudo');
      }
...
HciBle.prototype.startAdvertising = function(name, serviceUuids) {
  ...
  if (serviceUuids && serviceUuids.length) {
    for (i = 0; i < serviceUuids.length; i++) {
      var serviceUuid = new Buffer(serviceUuids[i].match(/.{1,2}/g).reverse().join(''), 'hex');
       debug('serviceUuid.length= ' + serviceUuid.length)
        debug("serviceUuid["+i+"] string:" +serviceUuids[i] +" hex:" +serviceUuid.toString('hex'))
  ...
  if (serviceUuids16bit.length) {
      debug("serviceUuids16bit.length: "+ serviceUuids16bit.length)
    advertisementDataLength += 2 + 2 * serviceUuids16bit.length;
      debug("advertisementDataLength: " +advertisementDataLength)
  }

  if (serviceUuids128bit.length) {
       debug("serviceUuids128bit.length: "+ serviceUuids128bit.length)
